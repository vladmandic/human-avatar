...
function createCamera(name: string, scene: Scene) {
...
async function positionCamera(camera: Nullable<ArcRotateCamera>, x: number, y: number, z: number) {
  const steps = 30;
  if (!camera) return;
...
async function rotateCamera(camera: Nullable<ArcRotateCamera>, which: string, startAngle?: number, endAngle?: number) {
  if (!camera) return;
...
async function zoomCamera(camera: Nullable<ArcRotateCamera>, radius: number) {
  if (!camera) return;
